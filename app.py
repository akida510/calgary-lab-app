...
st.markdown("""
    ...
    .invoice-paper {
        ...
        min-height: 1000px; padding: 45px; 
    ...
    <div class="slim-dashboard">
        ...
        <div style="display: flex;">
            <div class="money-badge" style="border-color: #555;">
                ...
                <span class="money-text" style="color: #eee;">${total_count * PRE_TAX_UNIT:,.2f}</span>
            </div>
            <div class="money-badge">
                <span class="money-label-sub">AFTER-TAX</span>
                <span class="money-text">${total_count * POST_TAX_UNIT:,.2f}</span>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("💾 케이스 저장"):
        if cln != "선택" and case_no:
            ...
            st.session_state.db.append({
                ...
                "Due": str(due), "Date": date.today().strftime('%m/%d/%Y')
            })
        ...
        <div class="invoice-paper">
            ...
            <div class="invoice-footer">
                <div style="display:flex; justify-content:space-between; font-size:14px; border-top:1px solid #ddd; padding-top:10px; margin-bottom:10px;">
                    <div style="color:#666;">Case: {inv['Case No']}</div><div style="font-weight:bold;">Total: $180.00</div>
                </div>
