...
st.markdown("""
    ...
    .invoice-outer-wrapper {
        ...
        padding: 10px 0;
        background-color: #333; /* 폰에서 종이가 돋보이게 배경색 설정 */
    }
    ...
    /* 모바일에서 레터지 비율을 유지하며 화면에 맞게 축소 */
    @media screen and (max-width: 8.5in) {
        .invoice-outer-wrapper { padding: 0; background-color: transparent; }
        .invoice-container {
            transform: scale(calc(100vw / 850)); /* 화면 너비에 맞춰 전체 축소 */
            transform-origin: top center;
            margin-bottom: calc(-11in * (1 - (100vw / 850))); /* 축소된 만큼 여백 제거 */
        }
    }
...
with tab2:
    if not st.session_state.db:
        ...
        for i, row in enumerate(st.session_state.db):
            ...
            with c_btn:
                if st.button("완료 및 인보이스", key=f"p_{i}"):
    ...
    if st.session_state.selected_invoice:
        ...
        <div class="invoice-outer-wrapper">
            <div class="invoice-container">
                ...
                <div style="margin: 40px 0 10px 0; font-size: 16px; border-bottom: 2px solid black; padding-bottom: 5px;"><b>Patient:</b> {str(inv['Patient']).upper()}</div>
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tr>
                        <td style="padding: 20px 0; height: 350px; vertical-align: top; font-size: 14px;">Nightguard ({inv['Material']}) {inv['Arch'].upper()}</td>
                ...
                <div style="margin-top: 80px; text-align: center;">
                    <div style="font-size: 12px; font-weight: bold; text-decoration: underline; margin-bottom: 10px;">All dental products we offer are custom made in Canada.</div>
                    <div style="font-size: 8.5px; line-height: 1.4; color: #444 !important; text-align: center;">
