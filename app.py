...
try:
    ...
    if not main_df.empty:
        main_df['Shipping Date'] = pd.to_datetime(
            main_df['Shipping Date'], errors='coerce'
        )
...
with t1:
    with st.expander("1️⃣ 기본 정보 (필수)", expanded=True):
        ...
        with c2:
            cl_list = sorted([c for c in ref_df.iloc[:, 1].unique() 
                             if c and c.lower() not in ['nan', 'clinic']])
    ...
    with st.expander("3️⃣ 체크 및 메모"):
        opts = sorted(list(set([i for i in ref_df.iloc[:, 3:].values.flatten() 
                               if i and i.lower() not in ['nan', 'none', '']])))
