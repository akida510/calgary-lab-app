...
with tab1:
    ...
    if submit_button:
        ...
        if not actual_clinic or not patient or actual_clinic == "선택하세요":
            ...
            try:
                ...
                st.success(f"✅ {patient}님 케이스 저장 완료! 화면을 새로고침합니다.")
                st.balloons()
                st.cache_data.clear()
                st.rerun() # rerun 시 st.form의 clear_on_submit=True 덕분에 깨끗이 비워짐
            except Exception as e:
                st.error(f"저장 실패: {e}")
