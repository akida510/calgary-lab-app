...
with tab1:
    ...
    with c2:
        ...
        if is_3d:
            st.text_input("접수일", value="-", disabled=True)
            f_rec_date = "-"
        else:
            f_rec_date = st.date_input("접수일", value=date.today())
            ...
            <div class="invoice-paper">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px;">
                    <div>
                        ...
                        <h1 style="font-size: 50px; font-weight: 900; font-style: italic; color: #1a4e8a; margin:0; line-height:1;">skycad</h1>
                    </div>
                    <div style="text-align: right;">
                        <h1 style="font-size:38px; font-weight:500; margin:0 0 10px 0;">INVOICE</h1>
                        <p style="margin:0; font-size:14px;">No. {inv['Inv_No']}</p>
                        <p style="margin:0; font-size:14px;">{date.today().strftime('%-m/%-d/%Y')}</p>
                    </div>
                </div>
                <div style="margin: 20px 0; padding: 12px 0; border-top: 2px solid black; border-bottom: 2px solid black; font-size: 18px; font-weight: bold;">
                    Patient: &nbsp; {inv['Patient'].upper()}
                </div>
                <table style="width: 100%; border-collapse: collapse; flex-grow: 1; margin-bottom: 20px;">
                    <thead><tr><th style="text-align:left; border-bottom: 1.5px solid black; padding: 8px 0;">Description</th><th style="text-align:right; border-bottom: 1.5px solid black; padding: 8px 0;">Amount</th></tr></thead>
                    <tbody><tr><td style="padding:20px 0; font-size: 16px;">Nightguard ({inv['Material']}) {inv['Arch']}</td><td style="text-align:right; font-size: 16px;">$180.00</td></tr></tbody>
                </table>
                <div style="margin-top: auto; display:flex; justify-content:space-between; font-weight:bold; font-size:18px; border-top:1px solid #eee; padding-top:10px;">
