...
with tab2:
    if not st.session_state.db: st.info("대기 중인 케이스가 없습니다.")
        ...
        for i, row in enumerate(st.session_state.db):
            ...
            with c_btn:
                if st.button("인보이스 보기", key=f"inv_btn_{i}"):
    ...
    if st.session_state.selected_invoice:
        ...
        <div class="invoice-container">
            ...
            <div style="margin-top:70px; border-top:1px solid black; width:220px; padding-top:10px; font-size:13px;">Authorized Signature</div>
        </div>
        """, unsafe_allow_html=True)
