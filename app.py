...
st.markdown(f"""
    ...
    <div class="slim-dashboard">
        <div><span style="color:#94a3b8;">{current_month} 실적:</span> <span style="color:white; font-weight:bold;">{len(st.session_state.db)} / 320</span></div>
        <div style="display: flex;">
            <div class="money-badge" style="margin-left:15px;"><span style="color:#4ade80;">AFTER-TAX: ${len(st.session_state.db)*POST_TAX_UNIT:,.2f}</span></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    # 인보이스 활성화 상태면 등록창 진입 시 초기화 (등록창에는 인보이스 안 뜨게)
    ...
    with st.form("case_form", clear_on_submit=True):
        ...
        with c1:
            ...
            # 병원명 선택 (직접 입력 통합)
            sel_cln = st.selectbox("Clinic (병원명)", ["선택하세요", "직접 입력"] + all_clinics)
            final_cln = ""
            if sel_cln == "직접 입력":
                final_cln = st.text_input("병원명 입력")
            else:
                final_cln = sel_cln
            ...
            sel_dr = st.selectbox("Dr (의사명)", ["선택하세요", "직접 입력"] + all_doctors, 
                                  index=all_doctors.index(auto_dr)+2 if auto_dr in all_doctors else 0)
            final_dr = ""
            if sel_dr == "직접 입력":
                final_dr = st.text_input("의사명 입력")
            else:
                final_dr = sel_dr
        ...
        if st.form_submit_button("💾 정보 저장 및 초기화"):
            if case_no and final_cln != "선택하세요":
...
with tab2:
    ...
    for i, row in enumerate(st.session_state.db):
        ...
        with col1:
            st.write(f"**No. {row['Inv_No']}** | {row['Patient']} ({row['Clinic']}) - {row['ModelInfo']}")
        with col2:
            ...
            <div class="inv-paper">
                ...
                <div style="flex: 1; margin-top: 30px;">
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody><tr><td style="padding:25px 0; font-size:14px;">Nightguard ({inv['Material']}) {inv['Arch']}</td><td style="text-align:right; font-size:14px; font-weight:bold;">$180.00</td></tr></tbody>
                    </table>
                </div>
