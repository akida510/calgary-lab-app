...
st.markdown("""
    ...
    /* Global Font Color */
    label p, .stMarkdown p, .stMetric p, .stTabs [data-baseweb="tab"] p, div, span { 
    ...
    /* [Invoice Frame] Matches your photo exactly */
    .invoice-card {
    ...
    @media print {
        ...
        .invoice-card { display: block !important; border: 2px solid black !important; padding: 40px !important; margin: 0 !important; }
    }
    </style>
...
tab1, tab2, tab3 = st.tabs(["📝 Case Registration", "📊 Work List", "🔍 Search"])

# --- Tab 1: Case Registration ---
with tab1:
    ...
    if st.button("💾 SAVE CASE (REGISTER)"):
        if sel_clinic == "Select" or not case_no:
            ...
            st.session_state.db.append({
                ...
                "Due": due_date, "Addr": c_info['Addr'], "City": c_info['City'], "Phone": c_info['Phone']
            })
            st.success("Registration Successful!")

# --- Tab 2: Work List & Invoice ---
with tab2:
    if not st.session_state.db: st.info("No cases pending.")
        ...
        for i, row in enumerate(st.session_state.db):
            ...
            with c_btn:
                if st.button("Print Invoice", key=f"inv_{i}"):
