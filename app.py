...
# [1. 디자인 설정 - 버튼 크기 축소 및 톤다운된 세련된 다크 테마]
...
st.markdown("""
    ...
    /* 비활성화된 날짜창 스타일 */
    input:disabled { background-color: #21262d !important; color: #8b949e !important; border: 1px solid #30363d !important; }

    /* 버튼: 크기를 확 줄이고 색상을 세련된 다크그레이로 변경 */
    .stButton>button { 
    ...
    /* 인보이스 디자인 (레터지 규격) */
    .inv-outer-container {
        display: flex; justify-content: center; padding: 20px 0; background-color: #0d1117;
    }
    .invoice-letter {
...
def update_from_clinic():
    if st.session_state.clinic_key != "선택":
        doc = ref_data[ref_data['Clinic'] == st.session_state.clinic_key]['Doctor'].iloc[0]
        st.session_state.sel_doctor = doc
        st.session_state.sel_clinic = st.session_state.clinic_key
    else:
        st.session_state.sel_clinic = "선택"; st.session_state.sel_doctor = "선택"

def update_from_doctor():
    if st.session_state.doctor_key != "선택":
        cln = ref_data[ref_data['Doctor'] == st.session_state.doctor_key]['Clinic'].iloc[0]
...
# [3. 메인 화면]
st.markdown(f'<div style="padding:15px 0;"><span style="font-size: 22px; font-weight: 700;">🦷 Skycad Lab Night Guard Manager</span></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    with c1:
        ...
        st.selectbox("Clinic", cln_list, key="clinic_key", 
                     index=cln_list.index(st.session_state.sel_clinic), on_change=update_from_clinic)
        
        doc_list = ["선택"] + sorted(ref_data['Doctor'].tolist())
        st.selectbox("Doctor", doc_list, key="doctor_key",
                     index=doc_list.index(st.session_state.sel_doctor), on_change=update_from_doctor)
                     
    with c2:
        is_3d = st.checkbox("3D Model", value=True)
        today = date.today()
        # 3D모델 체크 시 날짜 고정 및 비활성화
...
with tab2:
    ...
    for i, row in enumerate(st.session_state.db):
        ...
        with c_info:
            ...
            st.markdown(f"**{status_icon} {row.get('Case No')}** | {row.get('Patient')} | {row.get('Clinic')}")
        with c_btn:
            btn_label = "완료" if row.get('Status') == "Pending" else "재출력"
            ...
            <div class="invoice-letter">
                <div style="display: flex; justify-content: space-between;">
                    ...
                    <div style="text-align: right;">
                        ...
                        <div style="text-align:left; font-size:11px; margin-top:15px;"><b>Ship To:</b><br>{inv.get('Clinic','')}<br>{inv.get('Address','')}</div>
                    </div>
                </div>
    ...
    <div class="stat-card">
        <div style="display: flex; gap: 50px;">
            <div><p style="margin:0; font-size:12px; color:#8b949e;">{this_month} 총 수량</p><p style="font-size:20px; font-weight:bold;">{total_count} / 320</p></div>
            ...
            <div><p style="margin:0; font-size:12px; color:#8b949e;">인센티브 (세후 $19.5)</p><p style="font-size:20px; font-weight:bold; color:#3fb950;">${over_count * 19.5:,.1f}</p></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
