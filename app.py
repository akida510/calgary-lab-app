...
st.markdown("""
    ...
    div[data-testid="column"] .stButton>button {
        height: 30px !important; font-size: 12px !important; padding: 0 10px !important;
    }
    /* 인보이스 컨테이너: 박스 제거 및 배경 흰색 */
    .invoice-container {
        background-color: white !important; color: black !important; 
        padding: 50px; font-family: 'Arial', sans-serif;
        max-width: 800px; margin: 0 auto; line-height: 1.4;
    }
    ...
    if st.button("💾 케이스 저장 (접수 완료)"):
        if sel_clinic == "선택" or not case_no:
            ...
            st.session_state.db.append({
                ...
                "Material": material, "Arch": arch, "Received": rec_date, "Due": due_date,
                "Lab Done": lab_done_date, "Status": "Pending", "Addr": c_info['Addr'], 
                "City": c_info['City'], "Phone": c_info['Phone']
            })
...
with tab2:
    if not st.session_state.db:
        ...
        for i, row in enumerate(st.session_state.db):
            ...
            with c_info: st.write(f"**{row['Case No']}** | {row['Patient']} | {row['Clinic']} | Due: {row['Due']}")
            with c_btn:
    ...
    if st.session_state.selected_invoice:
        ...
        st.divider()
        # HTML 코드 노출 방지를 위해 스타일을 분리하여 작성
        invoice_html = f"""
        <div class="invoice-container">
            <div style="display: flex; justify-content: space-between;">
                <div>
                    <div style="font-size: 10px; font-weight: bold; color: #1a4e8a !important;">DENTAL TECHNOLOGY Ltd</div>
                    <div style="font-size: 60px; font-weight: 900; font-style: italic; color: #1a4e8a !important; line-height: 0.7;">skycad</div>
                ...
                <div style="text-align: right;">
                    ...
                    <div style="margin-top: 20px; text-align: left; font-size: 13px; border: 1px solid black; padding: 10px; width: 220px; display: inline-block;">
                        <b>Ship To:</b><br>{inv['Clinic']}<br>Dr. {inv['Doctor']}<br>{inv['Addr']}<br>{inv['City']}<br>{inv['Phone']}
                    </div>
                </div>
            </div>
            <div style="margin: 50px 0 10px 0; font-size: 16px; border-bottom: 2px solid black; padding-bottom: 5px;">
                <b>Patient:</b> {str(inv['Patient']).upper()}
            </div>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <tr style="border-bottom: 1px solid black; font-weight: bold;">
                    <td style="padding: 8px 0; text-decoration: underline;">Description</td>
                    <td style="padding: 8px 0; text-align: right; text-decoration: underline;">Amount</td>
                </tr>
                <tr>
            ...
            <div style="margin-top: 60px; text-align: center; font-size: 14px;">
                <div style="font-weight: bold; text-decoration: underline; margin-bottom: 10px;">All dental products we offer are custom made in Canada.</div>
                <div style="font-size: 10px; line-height: 1.4; color: #444 !important;">
